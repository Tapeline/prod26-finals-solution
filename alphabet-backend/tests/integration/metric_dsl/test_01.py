...
from tests.integration.metric_dsl.helper import (
    insert_events, ok_evt,
    a_metric, now_window, waiting_evt, err_evt,
)


@pytest.mark.asyncio
async def test_good_events_selected(
    clickhouse_client,
    evaluator
):
    await insert_events(
...
async def test_sum_with_json_filters(clickhouse_client, evaluator):
    ...
    await insert_events(
        clickhouse_client,
        ok_evt(
            "purchase", "e2:f1:u1:v1", {"amount": 100, "currency": "USD"}
        ),
        ok_evt(
            "purchase", "e2:f1:u1:v1", {"amount": 50, "currency": "EUR"}
        ),
        ok_evt(
            "purchase", "e2:f1:u2:v2", {"amount": 20, "currency": "USD"}
        ),
    )
...
async def test_conversion_ratio(clickhouse_client, evaluator):
    await insert_events(
        ...
        ok_evt("view", "e3:f1:u1:v1"), ok_evt("view", "e3:f1:u2:v1"),
...
async def test_cross_source_metric(clickhouse_client, evaluator):
    ...
    metrics = [
        a_metric("ErrorRate", "count discarded view / count view")
    ]
    print(metrics)
...
async def test_advertised_example_with_ios(clickhouse_client, evaluator):
    await insert_events(
        clickhouse_client,
        ok_evt(
            "abc", "e5:f1:u1:v1", delivery_latency_ms=100,
            attributes={"device": {"platform": "iOS"}}
        ),
        ok_evt(
            "def", "e5:f1:u2:v1", delivery_latency_ms=200,
            attributes={"device": {"platform": "iOS"}}
        ),
        ok_evt(
            "ghi", "e5:f1:u3:v1", delivery_latency_ms=300,
