...
!!! caution
    Дедупликация событий необходимо для того, чтобы валидно обрабатывать случаи,
    когда, например, по причине ретраев или проблем с сетью сообщение дошло
    дважды. **Как правило**, такие запросы приходят друг за другом, а не
    параллельно. Текущая реализация будет иметь неопределённое поведение в случае,
    если будет сделано два конкурентных запроса на регистрацию одних и тех
    же событий. Сделано это с целью оптимизации (события приходят батчами и
    загрузка/сохранение в Redis, соответственно, тоже). Более конкретно, если
    случится так, что оба запроса успеют выполнить загрузку из Redis до того,
    как хотя бы один из них сделает сохранение полученных ID туда, то
    оба события будут приняты. **Это известное ограничение.**
...
!!! note
    События не сохраняются сразу, а накапливаются в батчи для лучшей производительности
    вставок в ClickHouse. На данный момент события накапливаются in-memory. Хотя это
    и просто реализуется с технической точки зрения, при падении сервиса, невысланные
    события будут теряться. В дальнейшем, планируется изменить механизм накопления,
    если того потребует продакшн (например, сохранять во временной таблице или
    отсылать сообщениями в брокера).
...
причина отклонения короткой строкой, по которой можно будет потом агрегировать статистику.
...
!!! note
    Если было отправлено "битое" событие с одним event_id, а затем за ним будет
    отправлено нормальное с тем же event_id, то последнее будет расценено системой
    как дубликат.
