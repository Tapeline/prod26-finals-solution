...
2. Воркер периодически вычитывает ожидающие оповещения, группирует их по правилам рассылки.
3. Воркер проверяет время последней отправки для группы.
   ...
   (форматирование по Jinja шаблону происходит на этом же этапе).

Отпечаток считается как ключ от ID Правила + ID Сущности, 
