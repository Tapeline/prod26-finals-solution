---
toc-depth: 3
---

# Feature flags

## Сущности

### Feature flag

{{ decorated_definition("exp", "Feature flag") }}

- <exp:Ключ флага>
- описание флага (необязательно)
- значение по умолчанию
- <exp:Тип флага>
- <access:ID> пользователя, создавшего флаг
- время создания флага
- время последнего изменения флага

### Эксперимент

{{ decorated_definition("exp", "Эксперимент") }}

- <exp:ID>
- название эксперимента
- <exp:Ключ флага>, затронутого экспериментом
- <exp:Статус>
- <exp:Версия>
- доля аудитории (число процентов)
- список <exp:Вариант|Вариантов>
- <exp:Правило таргетирования> (опционально)
- <access:ID> автора эксперимента
- время создания
- время последнего изменения
- итог эксперимента
- <exp:Метрика|Метрики> эксперимента из каталога
- <exp:Приоритет> (опционально)
- <exp:Конфликтный домен> (опционально)
- <exp:Политика разрешения конфликта> (опционально)

!!! note
    Проценты - целое число.

**Инварианты**

- сумма весов инвариантов равна 100 (%)
- ровно 1 вариант является контрольным
- ровно 1 эксперимент со статусом "запущен" на 1 флаг
- итог эксперимента присутствует тогда и только тогда, когда Статус = В архиве
- конфликтный домен и политика разрешения либо одновременно не заданы, либо
  одновременно заданы оба
- значения всех вариантов соответствуют типу данных флага

!!! note
    Условие определяет:

    > сумма весов по всем вариантам должна совпадать с долей аудитории эксперимента;

    Данное требование мешает удобно расширять общую аудиторию эксперимента.
    Им мы __пренебрежём__.

**Диаграмма статусов**:

```mermaid
stateDiagram
  D: Черновик
  R: На ревью
  A: Одобрен
  RUN: Запущен
  P: На паузе
  C: Завершён
  AR: В архиве
  X: Отклонён
  HOLD: Остановлен по сообра- \nжениям безопасности

  [*] --> D
  D --> R: отправить на ревью
  R --> A: "одобрить (порог достигнут)"
  R --> D: запросить доработки
  R --> X: отклонить
  X --> D: доработать и отправить заново

  A --> RUN: запустить
  RUN --> P: поставить на паузу
  P --> RUN: возобновить
  RUN --> C: завершить
  RUN --> HOLD: guardrail срабатывание
  HOLD --> C: завершить
  P --> C: завершить
  C --> AR: архивировать
  AR --> [*]
```

!!! note
    Не предусмотрено возврата из "Остановлен по соображениям безопасности"
    обратно в состояние "Запущен", так как:

    1. Это небезопасно, вариант может снова начать деградировать
    2. Даже если вариант уже исправлен, статистика испорчена деградацией

### Решение ревью

{{ decorated_definition("exp", "Решение ревью") }}

- <exp:ID> эксперимента
- Статус решения
- Опциональный комментарий (строка)
- Время принятия решения

## Объекты-значения

### Вариант

{{ decorated_definition("exp", "Вариант") }}

- название варианта `[A-Za-z0-9_-]+`
- значение варианта
- признак контрольного варианта (да/нет)
- вес варианта (доля аудитории варианта, в процентах)

!!! note
    Проценты - целое число.

### Итог эксперимента

{{ decorated_definition("exp", "Итог эксперимента") }}

- тип итога эксперимента
- комментарий (строка)

### Политика разрешения конфликта

{{ decorated_definition("exp", "Политика разрешения конфликта") }}

- один или никто
- наивысший приоритет

### Статус решения

{{ decorated_definition("exp", "Статус решения") }}

### Набор метрик

- первичная (имя)
- вторичные (несколько имён)
- охранительные (несколько имён)

!!! note
    Эксперимент не проверяет, существуют ли эти метрики. 
    Строго говоря, контекстом экспериментов это поле воспринимается
    как хранилище данных для контекста отчёта. Это известное ограничение.

## Сценарии

### Создание флага

**Кто**: <access:Экспериментатор>, <access:Администратор>

**Ограничения**:

- <exp:Ключ флага> уникален

**Шаги**:

1. Создание флага с указанным ключом, типом, значением по умолчанию и описанием
2. Система автоматически заполняет владельца флага и время создания и изменения

### Получение списка флагов

**Кто**: любой пользователь

### Просмотр конкретного флага

**Кто**: любой пользователь

### Обновление флага

**Кто**: <access:Экспериментатор>, <access:Администратор>

**Ограничения**:

- Обновлять можно только значение по умолчанию

### Создание эксперимента

**Кто**: <access:Экспериментатор>

**Шаги**:

1. Генерация <exp:ID> 
2. Проверка соответствия каждого варианта типу данных флага.
3. Создание эксперимента с указанным названием, ключом флага, долей аудитории, 
    вариантами, таргетированием и метриками
4. Статус = Черновик, Версия = 1

### Отправка на ревью

**Кто**: <access:Экспериментатор>

**Ограничения**:

- Статус = Черновик

**Шаги**:

1. Статус = На ревью
3. Выслать оповещение, если нужно

### Восстановление черновика

**Кто**: <access:Экспериментатор>

**Ограничения**:

- Статус = Отклонён

**Шаги**:

1. Статус = Черновик
3. Выслать оповещение, если нужно

### Отклонение черновика

**Кто**: <access:Утверждающий>, <access:Администратор>

**Ограничения**:

- <access:ID> Утверждающего в списке Утверждающих Экспериментатора 
- **ИЛИ** список утверждающих не задан и актор — Администратор
- Статус = На ревью

**Шаги**:

1. Статус = Отклонён
2. Удалить все факты одобрения для этого эксперимента
4. Выслать оповещение, если нужно
5. Сохранить решение

### Одобрить черновик

**Кто**: <access:Утверждающий>, <access:Администратор>

**Ограничения**:

- <access:ID> Утверждающего в списке Утверждающих Экспериментатора
- Статус = На ревью

**Шаги**:

1. Добавить одобрение для этого черновика
2. Если не заданы параметры утверждающих для этого экспериментатора:

    1. Если Актор — Администратор, то Статус = Одобрен
    3. Выслать оповещение, если нужно
    4. Сохранить решение

3. Иначе если одобрений >= необходимо для этого экспериментатора:
    
    1. Статус = Одобрен
    3. Выслать оповещение, если нужно
    4. Сохранить решение

!!! note
    Так как в иерархии ролей <access:Администратор> стоит выше, чем
    <access:Утверждающий> и Утверждающих назначает именно Администратор,
    то в отсутствие таковых назначенных Администратор берёт на себя
    ответственность за принятие решения (либо назначение Утверждающих для принятия решения).

### Запуск

**Кто**: <access:Экспериментатор>

**Ограничения**:

- Статус = Одобрен
- Нет уже запущенного или на паузе эксперимента с таким же Ключом флага

**Шаги**:

1. Статус = Запущен
3. Выслать оповещение, если нужно

### Управление запущенным экспериментом

**Кто**: <access:Экспериментатор>

**Ограничения**:

- Переход Запущен -> На паузе
- Переход На паузе -> Запущен
- Переход Запущен -> Завершён
- Переход На паузе -> Завершён
- Переход Остановлен по соображениям безопасности -> Завершён
- Нет уже запущенного или на паузе эксперимента с таким же Ключом флага

### Подведение итогов и архивация

**Кто**: <access:Экспериментатор>

**Ограничения**:

- Статус = Завершён

**Шаги**:

1. Создать нужный вариант Итога эксперимента с комментарием
2. Статус = В архиве
4. Выслать оповещение, если нужно

### Просмотр всех экспериментов

**Кто**: кто угодно

### Просмотр версии эксперимента

**Кто**: кто угодно

### Просмотр истории версий эксперимента

**Кто**: кто угодно

### Просмотр аудита эксперимента

**Кто**: кто угодно

**Шаги**:

1. Извлекается текущий список одобрений
2. Извлекается текущее <exp:Решение ревью|Решений ревью>
3. Информация отдаётся пользователю

### Изменение эксперимента

**Кто**: <access:Экспериментатор>

**Ограничения**:

- Статус == Черновик

!!! note
    Изменять можно только черновик, так как:

    - Изменять запущенный эксперимент по условиям нельзя.
    - Изменять эксперимент после согласования нельзя, так как
        могут быть внесены опасные изменения, которые не были бы согласованы.
    - Изменять остановленный и архивированный эксперимент нельзя,
        так как это нарушает историческую правдивость.

**Шаги**:

1. Проверка соответствия каждого варианта типу данных флага.
2. Изменить эксперимент в соответствии с новыми:
    - названием
    - аудиторией
    - ключом флага
    - списком вариантов
    - таргетированием
    - метриками
    - конфликтным доменом
    - политикой разрешения
    - приоритетом
3. Увеличить версию
